    ...
    try {
        ...
        state.search.results = data.data.recipes.map(rec => {
            return {
                ...
                image: rec.image_url,
                ...(rec.key && {key: rec.key})
            }; // this will return a new array with the new objects and we store that in the state
        });
        state.search.page = 1;
...
export const uploadRecipe = async function (newRecipe) {
    try {
        // Take the raw input data and transfrom it into the same format that we get out of the API
